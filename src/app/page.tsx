...
const Home: React.FC = () => {
  ...
  const [newTask, setNewTask] = useState({ title: '', description: '', columnId: 'todo', tags: ["Normal"], dueDate: new Date().toDateString(),createdAt: new Date().toDateString() });
/**
 ...
 */
  ...
  return (
    ...
    <Nav/>
    <div className="p-2">
      <h1 className="text-2xl font-bold mb-2 bg-gray-400 text-white p-2">Task Manager</h1>
      <div className="w-full md:w-1/2 mb-2 grid grid-cols-2 gap-2">
        <input
          type="text"
          placeholder="Task title"
          value={newTask.title}
          onChange={(e) => setNewTask({ ...newTask, title: e.target.value })}
          className="border p-2 mb-0 w-[100%] col-span-2"
        />
        <input
          type="text"
        ...
        <input
          ...
          className="border p-2 mb-0 w-[100%]"
        />
        <button onClick={handleAddTask} className="bg-blue-500 text-white p-2 w-[100%] rounded">
          Add Task
        </button>
      </div>
      <Board />
